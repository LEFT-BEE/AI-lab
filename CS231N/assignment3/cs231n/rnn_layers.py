...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    z = np.dot(x , Wx) + np.dot(prev_h , Wh)
    next_h = np.tanh(z)
...
def word_embedding_forward(x, W):
    ...
    out = W[x, :]  # array indexing 이부분 전혀모르겠음
    cache = x, W
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    _ , H = prev_h.shape
    hx = np.dot(x , Wx) + np.dot(prev_h  , Wh) + b
    i = sigmoid(hx[:,:H])
    f = sigmoid(hx[:,H:2*H])
    o = sigmoid(hx[: , 2*H:3*H])
    g = np.tanh( hx[:, 3*H:4*H])
    next_c = np.multiply(f , prev_c) + np.multiply(i , g)
    next_h = np.multiply(o , next_c)
    cache = (next_h , next_c , g , o, f, i ,hx ,prev_c , prev_h , Wh , Wx , x)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    next_h , next_c , g , o, f, i ,hx ,prev_c , prev_h , Wh , Wx , x = cache
    _,H= prev_h.shape
    do = np.multiply(dnext_h , np.tanh(next_c))
    dnext_c += dnext_h * (1-np.tanh(next_c)) * o
